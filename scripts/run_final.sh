...
predict_epoch_average() {
  ...
  "$PY" -m alpha_scoring.ensembling.average_sequence_predictions "${inputs[@]}" --output "$output/$final_name"
}
...
if [[ ! -f submission_catboost_v4.csv ]]; then
  "$PY" -m alpha_scoring.models.tabular.train_v4_submission > "$STATUS/train_v4.log" 2>&1
fi
if [[ ! -f submission_catboost_v5.csv ]]; then
  "$PY" -m alpha_scoring.models.tabular.train_v5_submission > "$STATUS/train_v5.log" 2>&1
fi
if [[ ! -f submission_lightgbm_v5.csv ]]; then
  "$PY" -m alpha_scoring.models.tabular.train_lightgbm_v5_submission > "$STATUS/train_lightgbm_v5.log" 2>&1
fi
log_step "train_tabular_models_done"

log_step "build_sequence_cache_start"
if [[ ! -f sequence_cache/manifest.json ]]; then
  "$PY" -m alpha_scoring.features.build_sequence_cache --force > "$STATUS/build_sequence_cache.log" 2>&1
fi
...
if [[ ! -f "$RUNS/temporal_full_seed271/epoch_10.pt" || ! -f "$RUNS/temporal_full_seed42/epoch_10.pt" ]]; then
  bash scripts/train_temporal_full.sh
fi
...
"$PY" -m alpha_scoring.ensembling.average_sequence_predictions \
...
if [[ ! -f tabular_cache/metadata.json ]]; then
  "$PY" -m alpha_scoring.features.build_tabular_cache --force > "$STATUS/build_tabular_cache.log" 2>&1
fi
